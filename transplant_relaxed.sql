COPY ( 
/* find min, average and max of transplant rate for each urgent status */
select status, min(transplant_rate), avg(transplant_rate), max(transplant_rate)
from 
    (
    /* find transplant rate (per 100 patient year) for each urgent status in each iteration */
...
group by w.status     
order by w.status
) TO 'C:\TSAM\PostgresSQLcode - Copy\logistic_relaxed_transplant_rate.csv' (format CSV); 