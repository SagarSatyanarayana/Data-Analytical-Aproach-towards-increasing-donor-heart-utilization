...
from 
    ...
    from 
        ...
        from 
            ...
            inner join 
            	public.tsam_relaxed_status v on (u.iter = v.iter and u.patientid = v.patientid and u.max_canhx_begin_dt = v.canhx_begin_dt)
    ...
    inner join 
        (
            ...
            from 
                ...
                from public.tsam_relaxed_status s inner join tsam_relaxed_death d on (s.iter = d.iter and s.patientid = d.patientid) 
                where s.patientid in (select wl_id from public.waitlist20092011_usc where can_age >= 18
                                      union select wl_id from public.patients20092011_usc where can_age >= 18) 
                ...
                from public.tsam_relaxed_status s inner join tsam_relaxed_graft g on (s.iter = g.iter and s.patientid = g.patientid) 
                where s.patientid in (select wl_id from public.waitlist20092011_usc where can_age >= 18
                                      union select wl_id from public.patients20092011_usc where can_age >= 18) 
                    and s.needheart = true and s.needlungs = false
...
) TO 'C:\TSAM\PostgresSQLcode - Copy\logistic_relaxed_mortality_rate.csv' (format CSV);